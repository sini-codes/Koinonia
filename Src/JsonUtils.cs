...
namespace Koinonia
{
    ...
    public static class JsonUtils
    {
            ...
            public static object DeserializeObject(Type type, JSONNode node)
            {
                ...
                {
                    var jsonArray = node.AsObject;
                    if (jsonArray == null) return null;

                    var itemKeyType = type.GetGenericArguments()[0];
                    var itemValueType = type.GetGenericArguments()[1];
                    var listType = typeof(Dictionary<,>).MakeGenericType(new[] { itemKeyType, itemValueType });
                    var dictionary = (IDictionary)System.Activator.CreateInstance(listType);

                    foreach (var instance in jsonArray)
                    {
                        var pair = (KeyValuePair < string, JSONNode> )instance;
                        var keyObject = DeserializeObject(itemKeyType, pair.Key);
                        var valueObject = DeserializeObject(itemValueType, pair.Value);
                ...
                if (typeof (IList).IsAssignableFrom(type)) // collection detected
                {
                    ...
                    {
                        var obj = DeserializeObject(itemType, instance);
                        list.Add(obj);
                    }
            ...
            public static object DeserializeExistingObject(object poco, JSONClass jsonClass)
            {
                ...
                {
                    if (propertyInfo.IsDefined(typeof (JsonProperty), true))
                    {
                        var jsonNode = jsonClass[propertyInfo.Name];

                        if (jsonNode == null) jsonNode = jsonClass[propertyInfo.Name.ToLower()];

                        if (jsonNode != null)
                        {
                            var deserializedObject = DeserializeObject(propertyInfo.PropertyType,
                                jsonNode);
    ...
    public class JSONNode
    {
        ...
        {
            get { return this as JSONArray ?? new JSONArray(); }
        }
        ...
        {
            get { return Quaternion.Euler(AsVector3); }
            set
            {
        ...
        {
            get { return new Vector3(this["x"].AsFloat, this["y"].AsFloat, this["z"].AsFloat); }
            set
            {
                var ob = this.AsObject;
                ob.Add("x", new JSONData(value.x));
